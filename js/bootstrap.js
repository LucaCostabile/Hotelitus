...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const carousel = new bootstrap.Carousel(document.getElementById('carouselHabitaciones'), {
      interval: 5000,
      wrap: true
  });

  // Prevenir que los clicks en los controles del carousel cierren la sección
  const carouselControls = document.querySelectorAll('.carousel-control-prev, .carousel-control-next');
  carouselControls.forEach(control => {
      control.addEventListener('click', function(e) {
          e.stopPropagation(); // Esto evita que el click llegue al contenedor padre
      });
  });
});
...
document.addEventListener('DOMContentLoaded', function() {
  const serviciosInfo = {
      'Desayuno gratuito': {
          caracteristicas: [
              '✓ Buffet completo',
              '✓ Opciones saludables y tradicionales',
              '✓ Horario extendido de 6:00 AM a 11:00 AM'
          ]
      },
      'Convenio con empresa de turismo': {
          caracteristicas: [
      ...
      'Servicio de lavandería': {
          caracteristicas: [
              ...
              '✓ Tratamiento especial para prendas delicadas'
          ]
      },
      'Posee pileta': {
          caracteristicas: [
  ...
  serviceCards.forEach(card => {
      card.addEventListener('click', function() {
          ...
          const caracteristicas = serviciosInfo[title]?.caracteristicas || [
              '✓ Disponible 24/7',
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Opcional: Agregar efecto de hover más suave
  cards.forEach(card => {
...
(function () {
  ...
  // Establecer fecha mínima para fecha de entrada (hoy)
  fechaEntrada.min = fechaActual;
  
  // Actualizar fecha mínima de salida cuando cambia la fecha de entrada
  fechaEntrada.addEventListener('change', function() {
      if(fechaEntrada.value) {
          // Obtener el día siguiente a la fecha de entrada
          const fechaMinSalida = new Date(fechaEntrada.value);
          fechaMinSalida.setDate(fechaMinSalida.getDate() + 1);
          fechaSalida.min = fechaMinSalida.toISOString().split('T')[0];
          
          // Si la fecha de salida es anterior a la nueva fecha mínima, la limpiamos
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  form.addEventListener('submit', function(event) {
      ...
      const formData = {
          ...
          fechaSalida: document.getElementById('inputFechaSalida').value,
          huespedes: document.getElementById('selectHuespedes').value,
          comentarios: document.getElementById('textareaComentarios').value
      };

      // Crear el cuerpo del correo
      const mailtoLink = `mailto:info@hotelitus.com.ar?subject=Nueva Solicitud de Reserva&body=
          Detalles de su reserva:%0D%0A
          %0D%0A
          Nombre: ${formData.nombre}%0D%0A
          DNI: ${formData.dni}%0D%0A
          Email: ${formData.email}%0D%0A
          Teléfono: ${formData.telefono}%0D%0A
          Fecha de entrada: ${formData.fechaEntrada}%0D%0A
          Fecha de salida: ${formData.fechaSalida}%0D%0A
          Número de huéspedes: ${formData.huespedes}%0D%0A
          Comentarios: ${formData.comentarios}%0D%0A
          %0D%0A
          Gracias por su solicitud de reserva. Nos pondremos en contacto con usted pronto.`;

      // Abrir el cliente de correo predeterminado
      window.location.href = mailtoLink;

      // Limpiar el formulario y quitar la validación
      form.reset();
